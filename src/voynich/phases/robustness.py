...
from voynich.core.utils import ensure_output_dir
...
from voynich.modules.phase12.budgeted_csp import BudgetedCSPDecoder, HUMORAL_VOCAB
from voynich.modules.phase12.syntactic_scaffolder import (
    SyntacticScaffolder, build_pos_transition_matrix,
)
from voynich.modules.phase12.ngram_mask_solver import NgramMaskSolver
from voynich.modules.phase12.char_ngram_model import LatinCharNgramModel
from voynich.modules.phase12.cross_folio_consistency import CrossFolioConsistencyEngine

from voynich.core.botanical_identifications import PLANT_IDS
...
def _build_illustration_prior_safe():
    if not ENABLE_ILLUSTRATION_PRIOR:
        return None
    try:
        from voynich.core.folio_illustration_priors import build_illustration_prior
...
def _normalize_section(section: str, folio: str) -> str:
    from voynich.core.voynich_corpus import _infer_section
    if section == 'herbal':
...
def _get_folio_metadata(folio: str, page=None) -> Dict:
    from voynich.core.voynich_corpus import _infer_section, _infer_language, _infer_scribe
    if page is not None:
...
def _compute_content_metrics(final_translations, latin_skel):
    ...
    """
    from voynich.core.expanded_medical_vocabulary import ALL_MEDICAL_CATEGORIES
    from voynich.modules.phase11.csp_decoder import FUNCTION_WORDS
...
def run_robustness_tests(
    ...
    if 'skeleton' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.skeleton_length_analysis import SkeletonLengthAnalysis
    ...
    if 'reversed' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.reversed_text_test import ReversedTextTest
    ...
    if 'consistency' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.consistency_significance import ConsistencySignificance
    ...
    if 'sensitivity' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.parameter_sensitivity import ParameterSensitivity
    ...
    if 'bootstrap' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.bootstrap_confidence import BootstrapConfidence
    ...
    if 'bidirectional' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.consistency_significance import BidirectionalConsistency
    ...
    if 'baselines' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.multiple_baselines import MultipleBaselines
    ...
    if 'ablation' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.ablation_cascade import AblationCascade
    ...
    if 'grille' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.cardan_grille_test import CardanGrilleTest
    ...
    if 'loo' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.leave_one_out import LeaveOneOutValidation
    ...
    if 'discriminant' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.discriminant_analysis import DiscriminantAnalysis
    ...
    if 'selectivity' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.selectivity_analysis import SelectivityAnalysis
    ...
    if 'selective_matching' in run_tests:
        if verbose:
            ...
            print('=' * 70)
        from voynich.modules.robustness.selective_matching_test import SelectiveMatchingTest
